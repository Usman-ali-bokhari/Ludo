...
void DrawArray(int x, int y, int width, int height, char arr[100],
float*color) {
	float fx = (float)x / width * 2 - 1, fy = (float)y / height * 2 - 1;
	DrawArray(fx, fy, arr, color);
}
// Function draws a string at given x,y coordinates
void DrawArray(float x, float y, char arr[100], float * color) {
	//  A pointer to a font style..
	//  Fonts supported by GLUT are: GLUT_BITMAP_8_BY_13,
	//  GLUT_BITMAP_9_BY_15, GLUT_BITMAP_TIMES_ROMAN_10,
	//  GLUT_BITMAP_TIMES_ROMAN_24, GLUT_BITMAP_HELVETICA_10,
	...
	//  Draw the characters one by one
	for (int i = 0; i < 100; i++)
		glutBitmapCharacter(font_style, arr[i]);
	glPopMatrix();
}
